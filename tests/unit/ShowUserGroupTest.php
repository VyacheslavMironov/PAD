...
class ShowUserGroupTest extends \Codeception\Test\Unit
{
    ...
    protected function _before()
    {
        ...
        // Просмотр студента в группе
        $userShowGroupTest = new UserGroupShowRepository();
        $response_show_group_user = $userShowGroupTest->show(new UserGroupShowDTO(
            $response_create_organization[3]['organization_id'],
            $response_filial_create['id'],
            $response_create_group['id']
        ));
        ...
        // Проверка параметров возвращаемого массива
        $this->assertNotNull($response_show_group_user);
        $this->assertEquals(11, count($response_show_group_user));
    }
